...
require_relative './../../Models/plugins/adapters/jenkins.rb'
require_relative 'jenkinsflatstrategy.rb'

class JenkinsRestPlugin < PluginModule::Plugin
    ...
    [
      {
        ...
        :klass => JenkinsRestPluginModule::JenkinsFlatStrategy,
        :type => :flat
      }
    ]
  end
  ...
  def show_summary_data(application, name, test, id, test_id, options=nil)
    ...
    metric = JenkinsRestPlugin.show_plugin_names.find {|i| i[:id] == id }
    results = {}
    if metric
      store = Redis.new(@db)
