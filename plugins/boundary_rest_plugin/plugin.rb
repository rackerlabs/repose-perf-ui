...
require_relative './../../Models/plugins/adapters/boundary.rb'
require_relative 'boundarytimeseriesstrategy.rb'

class BoundaryRestPlugin < PluginModule::Plugin
  ...
  def self.show_plugin_names
    [
      {
        :id => 'boundary',
        :name => 'Boundary Time Series Metrics',
        :klass => BoundaryRestPluginModule::BoundaryTimeSeriesStrategy,
        :type => :time_series
      }
    ]
  end
  ...
  def show_summary_data(application, name, test, id, test_id, options=nil)
    metric = BoundaryRestPlugin.show_plugin_names.find {|i| i[:id] == id }
    results = {}
    if metric
  ...
  def store_data(application, sub_app, type, json_data, store, start_test_data, end_time, storage_info)
    begin
      if json_data.has_key?('plugins')
        plugin_data = json_data['plugins'].find {|p| p['id'] == 'boundary_rest_plugin'}
        if plugin_data
          ...
          if servers
            servers.each do |server|
              PluginModule::Adapters::BoundaryRestAdapter.new(store, 'boundary_rest_plugin', server, storage_info).load(json_data['guid'], plugin_type, application, sub_app, type, start_test_data['time'], end_time)
            end
          else
            raise ArgumentError, "no server list specified"
          end
        else
          raise ArgumentError, "boundary_rest_plugin id not found"
        end
      end
      return nil
    rescue => e
      p e.backtrace
      return {'boundary_rest_plugin' => e.message}
    end
  end
end
