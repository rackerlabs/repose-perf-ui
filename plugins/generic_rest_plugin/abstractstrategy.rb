    ...
    def initialize(db, fs_ip, application, name, test_type, id, metric_id)
      ...
      begin 
        ...
        @data_results.each do |key, data_result|
          if key.start_with?("generic_rest_plugin")
            #load the file
            json_file = JSON.parse(meta_result)
            entry = JSON.parse(data_result)['location']
            name = JSON.parse(data_result)['name']
            template = JSON.parse(data_result)['template']
            
            results[json_file['name']] = populate_metric("http://#{fs_ip}/#{entry}", name, id, template, json_file['start'], json_file['stop']) if json_file
          end
        end
      ensure
        @store.quit
