  ...
  module Apps
    class FileSingularAppBootstrap < Bootstrap
   
  
      def initialize(environment = :production, redis_info = nil, logger = nil)
        if environment == :production
          @config = YAML.load_file(File.expand_path("config/apps/file_singular_app.yaml", Dir.pwd))
        elsif environment == :test
          @config = YAML.load_file(File.expand_path("config/apps/test_file_singular_app.yaml", Dir.pwd))
        elsif environment == :development
          @config = YAML.load_file(File.expand_path("config/apps/dev_file_singular_app.yaml", Dir.pwd))
        end
      ...
      def retrieve_adhoc_view(name = nil)
        if name && name == 'custom'
          "file_singular_app_custom_adhoc".to_sym
        else
          "file_singular_app_adhoc".to_sym  
        end
      end
